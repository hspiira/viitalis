...
export const Route = createFileRoute('/labs')({
  beforeLoad: () => requireAuthBeforeLoad('/labs'),
  component: LabsPage,
})
...
function LabsPage() {
  ...
  const { data: items = [], isLoading, error } = useQuery({
    queryKey: ['labs', skip, limit, opts.tenantId],
    queryFn: () => apiGet<CatalogItem[]>(`/labs?skip=${skip}&limit=${limit}`, opts),
    enabled: !!opts.tenantId,
  })

  const createMutation = useMutation({
    mutationFn: (body: { name: string; code?: string }) =>
      apiPost<CatalogItem>('/labs', body, opts),
    onSuccess: () => {
  ...
  const uploadMutation = useMutation({
    mutationFn: (body: { items: Array<{ name: string; code?: string }> }) =>
      apiPost<{ created: number; failed: number; errors: Array<{ row: number; message: string }> }>('/labs/upload', body, opts),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['labs'] })
      setShowUpload(false)
      setUploadJson('')
  ...
  const handleUpload = (e: React.FormEvent) => {
    ...
    try {
      ...
      uploadMutation.mutate({
        items: parsed.map((r) => ({ name: String(r.name ?? '').trim(), code: r.code ? String(r.code).trim() : undefined })),
      })
    } catch (err) {
    ...
    <div className="px-8 py-12 max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => { setShowUpload(false); setShowForm((v) => !v) }} className="bg-[var(--primary)] text-[var(--primary-foreground)] px-4 py-2 text-sm font-medium">{showForm ? 'Cancel' : 'Add item'}</button>
          <button type="button" onClick={() => { setShowForm(false); setShowUpload((v) => !v) }} className="border border-[var(--border)] px-4 py-2 text-sm">{showUpload ? 'Cancel' : 'Bulk upload'}</button>
        </div>
      </div>
      {showForm && (
      ...
      {showUpload && (
        <form onSubmit={handleUpload} className="mb-6 p-4 border border-[var(--border)] bg-[var(--card)] space-y-3">
          ...
          <textarea value={uploadJson} onChange={(e) => setUploadJson(e.target.value)} rows={8} className="w-full border border-[var(--input)] bg-[var(--background)] px-2.5 py-1.5 text-[var(--foreground)] font-mono text-sm" placeholder='[{"name":"Full blood count","code":"FBC"}]' />
