...
import { FlaskConical, Search, Plus, Upload } from 'lucide-react'
...
function LabsPage() {
  ...
  const {
    ...
    queryFn: () => apiGet<CatalogItem[]>(`/labs?${params}`, opts),
    ...
    return (
      <div className="w-full">
        ...
        <p className="text-[var(--foreground-muted)]">
          Sign in and select a tenant to manage the lab tests catalog.
        </p>
      </div>
    )
  }
      ...
      {isLoading ? (
        ...
        <Empty className="border border-[var(--border)] rounded-lg py-8 shadow-none mt-2">
          <EmptyHeader>
            ...
            <EmptyTitle>No lab tests</EmptyTitle>
            <EmptyDescription>
              No lab tests match your search. Add one or bulk upload.
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Button
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Add a lab test to the catalog.</DialogDescription>
        </DialogHeader>
        <form
          onSubmit={(e) => {
            e.preventDefault()
        ...
        >
          <div>
            <label htmlFor="lab-name" className="block text-sm text-[var(--foreground-muted)] mb-1">
              Name *
            </label>
            <input
              id="lab-name"
          ...
          <div>
            <label htmlFor="lab-code" className="block text-sm text-[var(--foreground-muted)] mb-1">
              Code
            </label>
            <input
              id="lab-code"
          ...
          <div>
            <label htmlFor="lab-remarks" className="block text-sm text-[var(--foreground-muted)] mb-1">
              Remarks
            </label>
            <input
              id="lab-remarks"
...
function BulkUploadLabsDialog({
  ...
  const uploadMutation = useMutation({
    mutationFn: (body: { items: Array<{ name: string; code?: string; remarks?: string }> }) =>
      apiPost<{
        created: number
        failed: number
        errors: Array<{ row: number; message: string }>
      }>('/labs/upload', body, opts),
    onSuccess: (data) => {
      setUploadJson('')
      setUploadError(null)
      queryClient.invalidateQueries({ queryKey: ['labs'] })
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Bulk upload lab tests</DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <textarea
            ...
            rows={8}
            placeholder='[{"name":"Full blood count","code":"FBC"}]'
