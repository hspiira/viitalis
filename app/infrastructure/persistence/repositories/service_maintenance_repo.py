"""ServiceMaintenance catalog repository. Tenant-scoped."""
...
def _to_result(m: ServiceMaintenance) -> CatalogItemResult:
    return CatalogItemResult(id=m.id, tenant_id=m.tenant_id, name=m.name, code=m.code)


class ServiceMaintenanceRepository:
    ...
    async def get_by_id(self, item_id: str) -> CatalogItemResult | None:
        r = await self.db.execute(
            select(ServiceMaintenance).where(
                ServiceMaintenance.id == item_id,
    ...
    async def list_by_tenant(self, skip: int = 0, limit: int = 100) -> list[CatalogItemResult]:
        r = await self.db.execute(
            select(ServiceMaintenance)
            .where(ServiceMaintenance.tenant_id == self.tenant_id)
            .offset(skip)
            .limit(limit)
            .order_by(ServiceMaintenance.name)
        )
        return [_to_result(m) for m in r.scalars().all()]

    async def create(self, data: CatalogItemCreate) -> CatalogItemResult:
        m = ServiceMaintenance(
