"""Lab catalog repository. Tenant-scoped."""
...
def _to_result(m: Lab) -> CatalogItemResult:
...
class LabRepository:
    ...
    async def get_by_id(self, item_id: str) -> CatalogItemResult | None:
        r = await self.db.execute(
            select(Lab).where(Lab.id == item_id, Lab.tenant_id == self.tenant_id)
        )
    ...
    async def list_by_tenant(self, skip: int = 0, limit: int = 100) -> list[CatalogItemResult]:
        r = await self.db.execute(
            select(Lab)
            .where(Lab.tenant_id == self.tenant_id)
            .offset(skip)
            .limit(limit)
            .order_by(Lab.name)
        )
